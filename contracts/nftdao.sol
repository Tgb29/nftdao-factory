...
contract NFTDAO is ERC721{
    ...
    mapping(uint => mapping(address => uint)) public voterHistory;
    ...
    function propose (uint _amount, string memory _ipfsHash) public {
        ...
        lastProposal[msg.sender] = _propsal.id;
        totalProposals +=1;
        proposerHistory[msg.sender].push(_propsal.id);

        emit ProposalCreated(msg.sender, _propsal.id);
    ...
    function executeProposal(uint _proposalId) public {
        ...
        if (totalVotes <= totalVotesThreshold) {
            ...
            if (proposals[_proposalId].yesVotes > proposals[_proposalId].noVotes) {
                ...
                (bool sent, bytes memory data) = _to.call{value: proposals[_proposalId].amount}("");
    ...
    //vote
    function vote(bool _yes, uint _proposalId) {
